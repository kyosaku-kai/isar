...
merge_wic_sbom() {
    ...
    # The per-rootfs sbom is emitted by sbom.bbclass as
    # ${PN}-${DISTRO}-${MACHINE}.<bomtype>.json (see generate_sbom). Earlier
    # versions of this function looked it up via ${IMAGE_FULLNAME}, which silently
    # broke whenever a downstream set IMAGE_FULLNAME:append (e.g. timestamp or
    # variant suffix), because the lookup name no longer matched the emitted
    # name. The cat then produced empty stdin, and debsbom 0.5.x silently wrote a
    # degenerate merge document. Since debsbom 0.6.1, an empty input list raises
    # IndexError ("readers[0].sbom_type()" in commands/merge.py), turning the
    # latent bug into a hard failure of do_image_wic.
    sbom_tmp=$(mktemp)
    cat ${DEPLOY_DIR_IMAGE}/${PN}-${DISTRO}-${MACHINE}.${bomtype}.json \
